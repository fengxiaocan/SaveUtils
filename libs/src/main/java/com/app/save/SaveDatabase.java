...
class SaveDatabase extends SQLiteOpenHelper{
    ...
    {
        super(context,"sFastSave.db",null,2);
    }
    ...
    public void onUpgrade(SQLiteDatabase db,int oldVersion,int newVersion){
        if(oldVersion == 1 && newVersion>oldVersion){
            db.execSQL(createUnique(STRING_TABLE));
            db.execSQL(createUnique(BOOLEAN_TABLE));
            db.execSQL(createUnique(INT_TABLE));
            db.execSQL(createUnique(LONG_TABLE));
            db.execSQL(createUnique(FLOAT_TABLE));
            db.execSQL(createUnique(DOUBLE_TABLE));
            db.execSQL(createUnique(BYTE_TABLE));
            db.execSQL(createUnique(SHORT_TABLE));
            db.execSQL(createUnique(BLOB_TABLE));
        }
    }

    private String create(final String table){
        return "CREATE TABLE "+table+"(id INTEGER PRIMARY KEY AUTOINCREMENT,"+KEY_NAME+" TEXT NOT NULL,"+VALUE_NAME+
               " TEXT,UNIQUE ("+KEY_NAME+"))";
    }

    private String createUnique(final String table){
        return "ALTER TABLE "+table+" ADD UNIQUE ("+KEY_NAME+"))";
    }
}
